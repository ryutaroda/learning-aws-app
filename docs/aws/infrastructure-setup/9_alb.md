...
4. ✅ **Route53ホストゾーン作成完了**: 使用するドメインのホストゾーンが作成済み（例: `ryutaroda.com`）
...
ALBをドメイン名でアクセスできるように、Route53でAレコードを作成します。

**前提条件**:
- Route53でホストゾーンが作成済みであること
- ドメインレジストラでネームサーバーがRoute53のものに設定されていること

- [x] **Route53でAレコードを作成**
  - Route53コンソール → ホストゾーン → `example.com`（または使用するドメイン）を選択
  - 「レコードを作成」をクリック
  - レコード設定:
    - **レコード名**: 空白（ルートドメインの場合）または `www`（サブドメインの場合）
    - **レコードタイプ**: `A - IPv4 address とルーティングトラフィックを`
    - **エイリアス**: `はい` を選択
    - **エイリアス先**: `Application and Classic Load Balancer` を選択
    - **リージョン**: `ap-northeast-1`（ALBと同じリージョン）
    - **ロードバランサーを選択**: `learning-alb-stg` を選択
    - **ルーティングポリシー**: `シンプルルーティング`
  - 「レコードを作成」をクリック

- [ ] **wwwサブドメインの設定（オプション）**
  - `www.example.com` でもアクセスできるようにする場合
  - 上記と同じ手順で、レコード名に `www` を指定してAレコードを作成

- [ ] **DNSプロパゲーションの確認**
  - レコード作成後、DNSの反映には数分〜数時間かかる場合があります
  - 確認コマンド:
    ```bash
    # DNS解決の確認
...
**重要**: Aレコードはエイリアスレコードとして作成します。エイリアスレコードを使用することで、ALBのIPアドレスが変更されても自動的に追従します。

#### 6. ECSサービスへのロードバランサー紐付け
